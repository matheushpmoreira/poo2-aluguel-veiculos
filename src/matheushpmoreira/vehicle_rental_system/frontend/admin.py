...
from matheushpmoreira.vehicle_rental_system.backend.models import Rental, Vehicle, VehicleStatus

from matheushpmoreira.vehicle_rental_system.frontend.base import Action, BaseFrame
from matheushpmoreira.vehicle_rental_system.frontend.choices import ChoiceBox, vehicle_status_choices, vehicle_status_filter_choices, vehicle_type_choices
from matheushpmoreira.vehicle_rental_system.frontend.form import VehicleForm
from matheushpmoreira.vehicle_rental_system.frontend.formatters import available_vehicle_choices, available_vehicle_filter, customer_admin_row, customer_choices, customer_payload, rental_admin_row, rental_payload, vehicle_admin_payload, vehicle_admin_row
...
class VehicleAdminFrame(BaseFrame):
    ...
    def _build(self) -> None:
        ...
        # Formulário
        form = VehicleForm(container, self.controller, self.create_vehicle)
        form.pack(side="top", fill="both", padx=(0, 10), pady=(0, 10))
        ...
        buttons.pack(side="top", fill="both", padx=(0, 10))
        ...
        self.table.bind("<<TreeviewSelect>>", lambda e: self.fill_from_selection(form))
    ...
    def create_vehicle(self) -> None:
        try:
            self.controller.post_vehicle(self.form.get_values())
    ...
    def fill_from_selection(self, form: VehicleForm) -> None:
        vehicle = self.table.selected_item()
        if vehicle is None:
            return
        values = asdict(vehicle)
        values["type"] = lang["vehicle"]["type"][vehicle.vehicle_type]
        values["status"] = lang["vehicle"]["status"][vehicle.status]
